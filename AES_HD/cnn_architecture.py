...
def loss_sca(score_vector, alpha_value=10, nb_class=256):
    ...
    def ranking_loss_sca(y_true, y_pred):
        ...
        # Get rank and scores related to the secret key (rank_sk = (?,))
...
# Log Initialization
sys.stdout = Logger(root+"logs/AES_HD_nbprof"+str(nb_prof_traces))
...
while(counter < 2):
    ...
    # Choose your model
    if(counter==0):
        model = cnn_architecture(input_size=input_size, learning_rate=learning_rate, alpha_value=alpha, rkl_loss=False)
        model_name="AES_HD_cce_nbprof"+str(nb_prof_traces)

    else:
        model = cnn_architecture(input_size=input_size, learning_rate=learning_rate, alpha_value=alpha, rkl_loss=True)
        model_name="AES_HD_rkl_nbprof"+str(nb_prof_traces)+"_alpha"+str(alpha)
    ...
    # Record the metrics
    history = train_model(X_profiling[:nb_prof_traces], Y_profiling[:nb_prof_traces], X_profiling[45000:], Y_profiling[45000:], model, AESHD_trained_models_folder + model_name, epochs=nb_epochs, batch_size=batch_size)
